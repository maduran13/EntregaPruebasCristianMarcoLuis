...
function create_integration() {
	...
	cy.get(".gh-btn").contains(new RegExp("Create")).click();
...
function opened_edit_integration(){
	...
	cy.get(".gh-btn").contains(new RegExp("Save")).click();
	cy.wait(100);
...
function edit_webhook(){
	cy.get('[class^="w6 h6 fill-midgrey pa1 mr1"]').click();
	cy.wait(200);
	...
	cy.wait(1000);
	cy.get(".gh-btn-green").contains(new RegExp("Save")).click();
