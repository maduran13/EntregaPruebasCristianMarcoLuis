...
let userIdentification = config.values.userIdentification;
let userPassword = config.values.userPassword;
...
console.log(inputValues);
  ...
  (async () => {
    ...
    for(b of browsers){
      ...
      await page.type('input[name="identification"]',userIdentification);
...
async function recursiveExploration(page, link, depth, parentState){
  ...
  if(!!body) {
    ...
    if(depth==0||link.includes(loginPage)){
    //if(link.includes(loginPage)){
      let elementList = []; 
